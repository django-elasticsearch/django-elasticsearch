...
class CarAutoFixture(AutoFixture):
    field_values = {
        ...
        "launched": generators.DateTimeGenerator(
            min_date=datetime(1950, 1, 1), max_date=datetime(2015, 12, 31)
        ),
        "type": generators.ChoicesGenerator(values=("se", "4x", "co", "br")),
    }
...
class ManufacturerAutoFixture(AutoFixture):
    field_values = {
        ...
        "created": generators.DateTimeGenerator(
            min_date=datetime(1900, 1, 1), max_date=datetime(2015, 12, 31)
        ),
        "country_code": generators.ChoicesGenerator(values=(COUNTRIES.keys())),
    }
