...
from django_elasticsearch.management.commands.search_index import Command
...
class SearchIndexTestCase(WithFixturesMixin, TestCase):
    def _mock_setup(self):
        ...
        patch_registry = patch(
            "django_elasticsearch.management.commands.search_index.registry",
            self.registry,
        )
    ...
    def test_get_models(self):
        cmd = Command()
        self.assertEqual(cmd._get_models(["foo"]), set([self.ModelA, self.ModelB]))
        ...
        self.assertEqual(
            cmd._get_models([]), set([self.ModelA, self.ModelB, self.ModelC])
        )
    ...
    def test_delete_foo_index(self):
        ...
        with patch(
            "django_elasticsearch.management.commands.search_index.input",
            Mock(return_value="y"),
        ):
            call_command(
                "search_index", stdout=self.out, action="delete", models=["foo"]
            )
